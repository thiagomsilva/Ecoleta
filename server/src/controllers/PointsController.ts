...
class PointsController {
    async index(request: Request, response: Response) {
        ...
        const parsedItems = String(items)
            .split(',')
            .map(item => Number(item.trim())); // Tira o espaço em branco que existir na busca do item
    ...
    async show(request: Request, response: Response) {
        ...
        const point = await knex('points').where('id', id).first(); // Busca um único ID no banco
